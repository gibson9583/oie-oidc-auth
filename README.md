...
**Requires the web administrator's Node deployment.** The browser-facing part of the flow — `/oidc/start`, `/oidc/callback`, and the client secret — lives in the web administrator's Node server. The **WAR deployment has no server half, so OIDC is not available with it**: the login card never offers SSO there, even with this extension installed and enabled. Local password sign-in is unaffected either way.
