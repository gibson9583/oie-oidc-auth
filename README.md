...
Three operations are pre-auth, because the login screen needs them before anyone is signed in: `/api/extensions/oidcauth/public` (enabled state, the button label and the auto-redirect flag — what the login card needs to decide whether to offer SSO, and nothing about the provider), `/start`, and `/callback`. The engine requires `X-Requested-With` on every API request, so the provider's redirect cannot land on the engine directly: the provider sends the browser to `<web-administrator-url>/oidc/callback`, a route of the web administrator, and the login card relays the returned `code` and `state` to `/callback`, which exchanges the code using the client secret and the PKCE verifier sealed in an HttpOnly cookie at `/start`, validates the ID token including its nonce, and answers with a one-time ticket. The card redeems the ticket through the ordinary `/users/_login` — `password=oidc:ticket:<id>` — so the session, the login audit event, and any second factor are exactly what a password sign-in gets. The client secret is stored in the engine's policy, shown masked in the tab, and never returned by the API. Only a ticket is a login credential: an ID token presented to `/users/_login` directly is refused without being examined. The code exchange authenticates with `client_secret_basic` — the method every provider must support — unless the provider's discovery document says it accepts only `client_secret_post`. The callback is rate-limited per client (the first `X-Forwarded-For` hop, which the web administrator's proxy sets and the engine already trusts for its audit log, else the peer address) and in total; nothing else in the flow costs the engine an outbound call, so nothing else is throttled.
...
**Replay protection is per-JVM.** The nonce check in the engine's callback ties every token to the attempt that asked for it, and the ticket that carries the token into the login is single-use; behind both, the engine remembers each accepted token for `max-token-age-seconds` plus `clock-skew-seconds` — the whole window in which it would still validate — so it cannot be accepted twice. That memory lives in the engine process: lost on restart, not shared between nodes. It is the backstop, not the mechanism, which is why a shared cache is deliberately out of scope for 1.0.
...
- **Deprovisioning is login-time only.** Removing a user at the IdP blocks their next sign-in, engine password included: an account bound to a provider subject refuses local sign-in while SSO is active (`This account signs in through SSO.`) unless it is listed in `linked-accounts`, and whenever SSO is off — policy disabled or the kill switch thrown — local passwords work for every account, which is the break-glass path. Removal does not disable or delete the engine account, and nothing sweeps for accounts whose IdP identity has gone. Remove the engine user as well.
